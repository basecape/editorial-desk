...
export async function runNewsAutopilot({ trigger = 'cron' } = {}) {
  ...
  let categoryIndex = 0;
  for (const categoryKey of enabledCategoryKeys) {
    ...
    try {
      ...
      await writeProgress({
        ...
        currentCategory: categoryKey,
        currentCategoryLabel: CATEGORY_LABELS[categoryKey] || categoryKey,
        currentStep: `found ${stories.length} stories, creating topics`,
        requestedForCategory: count,
        totalTopicsCreated: newTopics.length,
        errors: results.errors,
      });

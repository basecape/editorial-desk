...
Return ONLY the JSON array. No preamble, no commentary, no code fences.`;
  ...
  const text = await callAnthropic(prompt, { maxTokens: 4000, useWebSearch: true });
...
export async function runNewsAutopilot({ trigger = 'cron' } = {}) {
  ...
  if (!config.enabled) return { skipped: true, reason: 'autopilot disabled' };

  const lookbackHours = Number(config.lookbackHours) || 24;
  const autoWrite = config.autoWrite !== false; // default true
  ...
  for (const [categoryKey, catConfig] of Object.entries(targets)) {
    ...
    try {
      ...
      results.errors.push({ category: categoryKey, error: msg });
      results.perCategory[categoryKey] = { requested: count, retrieved: 0, created: 0, error: msg };
    }
  }
  ...
  // Update config with lastRun info
  const finalConfig = {
    ...config,
    lastRun: {

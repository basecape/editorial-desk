...
// Default 240s timeout to handle web-search + long-form article generation.
export async function callAnthropic(prompt, { maxTokens = 6000, useWebSearch = true, timeoutMs = 240000 } = {}) {
...
export async function generateArticle(topic, { settings = {}, categoryTraining = {}, sitePages = [], libraryItems = [] }) {
  ...
  // Web search only for news and mythbust — evergreen doesn't need current data
  const useWebSearch = topic.type === 'news' || topic.type === 'mythbusting';

  let text;
  try {
    text = await callAnthropic(prompt, { maxTokens: 6000, useWebSearch, timeoutMs: 240000 });
  } catch (e) {
    // If it timed out AND was using web search, fall back to no-search retry
    // This gets us SOME output rather than none for pathological queries
    const msg = String(e?.message || e);
    if (useWebSearch && (msg.includes('timeout') || msg.includes('AbortError'))) {
      // Retry without web search — much faster, more reliable
      text = await callAnthropic(prompt, { maxTokens: 6000, useWebSearch: false, timeoutMs: 120000 });
    } else {
      throw e;
    }
  }
